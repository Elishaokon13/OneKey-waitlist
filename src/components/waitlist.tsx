...
import { debounce } from "lodash"; // Import lodash debounce for input handling
...
export default function WaitlistModal({ children }: WaitlistModalProps) {
  ...
  // Get user agent safely on client side
  useEffect(() => {
    if (typeof window !== "undefined" && navigator?.userAgent) {
      // Sanitize user agent to prevent injection
      const sanitizedUserAgent = navigator.userAgent.replace(/[<>]/g, "");
      setUserAgent(sanitizedUserAgent.slice(0, 255)); // Limit length for storage
    }
  }, []);
  ...
  const handleSubmit = useCallback(
    ...
    [email, userAgent, toast, validateEmail]
  );
  ...
  const FormContent = () => (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-6" aria-label="Waitlist signup form">
      ...
      <div className="grid grid-cols-2 gap-4 pt-2">
        ...
        <motion.div
          ...
          transition={{ delay: 0.3 }}
        >
          <Zap className="w-4 h-4 text-primary" aria